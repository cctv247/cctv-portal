    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
      <div className="w-full max-w-md bg-white rounded-[50px] shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-8 bg-slate-50 border-t">
          <button disabled={selectedIds.length === 0} onClick={handleGenerate}
            className="w-full bg-blue-600 text-white font-[1000] py-6 rounded-[35px] shadow-xl active:scale-95 disabled:opacity-30 uppercase tracking-[3px] italic border-b-[6px] border-blue-900">
            <Printer size={20} /> Generate {selectedIds.length} Stickers
          </button>
        </div>
      </div>
    </div>
