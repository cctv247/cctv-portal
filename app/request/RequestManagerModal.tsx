...
export default function RequestManagerModal({ isOpen, onClose, onRefresh, filterSn }: any) {
  ...
  // 🚩 SAFARI & CHROME COMPATIBLE DISPATCH
  const handleAction = async (req: any) => {
    // 1. Data Prepare karein (Asynchronous kaam pehle khatam karein)
    ...
    try {
      // 2. Database update ko background mein bhejein (Safari fix)
      supabase.from("requests").update({ status: "done" }).eq("id", req.id).then(() => {
        fetchRequests(); 
        onRefresh();
      });

      // 3. Instant Redirect (Safari requires user action context)
      ...
      // Fallback: Agar upar wala fail ho toh anchor tag se try karein
