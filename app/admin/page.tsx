  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [devRes, reqRes] = await Promise.all([
        supabase.from("devices").select(`*, service_logs (next_service_date)`).order("site_name", { ascending: true }),
      ...
      if (devRes.data) {
        const processed = devRes.data.map((device: any) => {
          const validLogs = (device.service_logs || [])
          ...
          return { ...device, next_service_date: validLogs[0]?.next_service_date || null };
        });
