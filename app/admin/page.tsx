...
export default function AdminCentral() {
  ...
  const fetchDevices = async () => {
    ...
    // 💡 Devices table se sab data aur Service Logs se sirf dates uthana
    ...
    if (error) {
      ...
      // 💡 Har device ke liye Latest service log ki date nikalna
      const processed = data.map((device: any) => {
        const logs = device.service_logs || [];
        // Logs ko sort karke sabse latest (future/recent) date lena
        ...
        return {
          ...device,
          next_service_date: latestDate // Yeh UI Ribbon ke liye set ho gaya
        };
      });
      ...
      <div className="max-w-2xl mx-auto px-4 mt-8 space-y-7">
        {loading ? (
          ...
          filtered.map((device) => {
            ...
            if (diffDays !== null) {
              if (diffDays < 0) {
                alertStyle = "bg-[#ff3b30] text-white animate-pulse shadow-lg";
                statusText = `⚠️ LATE ${Math.abs(diffDays)}D`;
              } else if (diffDays === 0) {
                alertStyle = "bg-[#ff9500] text-white animate-bounce shadow-md";
                statusText = "🚨 TODAY";
              } else if (diffDays <= 3) {
                alertStyle = "bg-[#ffcc00] text-black font-black";
                statusText = `📅 ${statusText}`;
              } else {
                alertStyle = "bg-[#34c759] text-white";
                statusText = `📅 ${statusText}`;
              }
            }
            ...
            return (
              <div key={device.id} className="bg-white p-6 rounded-[40px] border border-slate-100 shadow-xl relative overflow-hidden transition-all hover:shadow-2xl pt-14 text-left">
                ...
                {nextDateStr && (
                  <div className={`absolute left-0 top-0 px-6 py-2.5 rounded-br-[25px] text-[10px] font-[1000] tracking-widest z-10 shadow-sm ${alertStyle}`}>
                    {statusText}
                  </div>
                )}

                <span className="absolute right-8 top-8 bg-blue-50/50 text-blue-600 px-4 py-1.5 rounded-2xl text-[9px] font-black uppercase tracking-widest border border-blue-100/30">
                  {device.category || 'DVR'}
                </span>

                <div className="space-y-4 mb-8">
                  <h3 className="text-[24px] font-[1000] text-slate-800 leading-none uppercase italic tracking-tighter">
                    {device.site_name}
                  </h3>
                  
                  <div className="flex items-center gap-3">
                    <div className="bg-slate-50 px-3.5 py-1.5 rounded-xl border border-slate-100 font-mono text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                      SN: {device.sn}
                    </div>
                    {parseFloat(device.latitude) ? (
                      <button onClick={() => trackOnMap(device)} className="p-2.5 bg-red-50 text-red-500 rounded-2xl border border-red-100 active:scale-90 shadow-sm transition-all">
                        <MapPin size={18} fill="currentColor" fillOpacity={0.2} />
                      </button>
                    ) : (
                      <div className="p-2.5 bg-slate-50 text-slate-300 rounded-2xl border border-slate-100">
