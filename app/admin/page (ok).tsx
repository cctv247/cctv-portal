...
import HistoryModal from "./HistoryModal"; // Ise import karein agar alag file hai
...
export default function AdminCentral() {
  ...
  const [isHistoryOpen, setIsHistoryOpen] = useState(false); // 📜 History State
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => router.push('/')}>
              ...
              <h1 className="text-xl font-[1000] text-slate-900 tracking-tighter italic">ADMIN CENTRAL</h1>
            </div>
          ...
          filtered.map((device) => (
            <div key={device.id} className="bg-white p-7 rounded-[45px] border border-slate-100 shadow-lg relative overflow-hidden transition-all duration-300 hover:shadow-2xl">
              ...
              <div className="mb-6 text-left">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {parseFloat(device.latitude) ? (
                    <button 
                      onClick={() => trackOnMap(device)}
                      className="p-2 bg-red-50 text-red-500 rounded-xl border border-red-100 active:scale-90 transition-all"
                    >
              ...
              <div className="flex items-center justify-between gap-3 mt-2 pt-4 border-t border-slate-50">
                ...
                {/* 📜 History Icon */}
                <button 
                  onClick={() => { setSelectedDevice(device); setIsHistoryOpen(true); }}
                  className="flex-1 flex flex-col items-center gap-1.5 p-2 rounded-[25px] hover:bg-slate-50 text-slate-400 hover:text-blue-600 transition-all active:scale-90"
                >
                ...
                {/* 🛠 Maintenance Icon */}
                <button 
                  onClick={() => router.push(`/engineer/${device.sn}`)}
                  className="flex-1 flex flex-col items-center gap-1.5 p-2 rounded-[25px] hover:bg-slate-50 text-slate-400 hover:text-emerald-600 transition-all active:scale-90"
                >
                  <div className="p-3 bg-slate-50 rounded-2xl">
                ...
                <button 
                  ...
                  className="flex-1 flex flex-col items-center gap-1.5 p-2 rounded-[25px] hover:bg-slate-50 text-slate-400 hover:text-orange-500 transition-all active:scale-90"
                >
                  <div className="p-3 bg-slate-50 rounded-2xl">
                    <Pencil size={20} />
                  </div>
                ...
                {/* 🔗 Portal Icon */}
                <button 
                  onClick={() => window.open(`/request/${device.sn}`, '_blank')}
                  className="flex-1 flex flex-col items-center gap-1.5 p-2 rounded-[25px] hover:bg-slate-50 text-slate-400 hover:text-indigo-600 transition-all active:scale-90"
                >
                  <div className="p-3 bg-slate-50 rounded-2xl">
                    <Globe size={20} />
                  </div>
