  ...
  return (
    <div className="fixed inset-0 z-[10000] bg-slate-900/90 backdrop-blur-md flex items-stretch sm:items-center justify-center p-0 animate-in fade-in duration-300">
      ...
      <div className="bg-[#f8fafc] w-full max-w-2xl h-[100dvh] sm:h-auto sm:max-h-[92vh] sm:rounded-[50px] shadow-2xl flex flex-col overflow-hidden relative animate-in slide-in-from-bottom duration-500 border-t border-white/20">
        ...
        {/* --- HEADER --- */}
        <div className="sticky top-0 z-[100] bg-white/80 backdrop-blur-xl border-b border-slate-200/50 p-5 flex justify-between items-center shrink-0 pt-[calc(env(safe-area-inset-top)+1rem)]">
          <div className="flex items-center gap-4 italic text-left">
            <div className="bg-blue-600 p-3 rounded-2xl text-white shadow-xl shadow-blue-100/50 active:scale-[0.98] transition-all">
          ...
          <button onClick={onClose} className="p-4 bg-slate-50 rounded-[20px] text-slate-400 active:scale-[0.90] border border-slate-100 transition-all shadow-sm">
            <X size={24} strokeWidth={3} />
          </button>
        </div>

        {/* --- BODY --- */}
        <div className="flex-1 overflow-y-auto px-6 sm:px-10 space-y-10 pt-8 pb-40 text-left overscroll-contain bg-[#f8fafc] touch-pan-y custom-scroll">
          
          {/* Identity */}
          <div className="space-y-3">
             <label className="text-[10px] font-[1000] uppercase text-slate-400 ml-1 tracking-[4px] leading-none flex items-center gap-2 italic uppercase">
               <Hash size={14} className="text-blue-500" /> Device SN
             </label>
             <div className="p-8 bg-white border-2 border-slate-100 border-dashed rounded-[35px] font-mono font-black text-slate-500 text-sm text-center shadow-inner italic break-all">
               {device.device_sn}
             </div>
          </div>
          ...
          {/* 🔐 SECURITY CARD (V-CODE ADDED HERE) */}
          <div className="bg-white p-8 rounded-[45px] border border-slate-100 shadow-xl space-y-8 relative overflow-hidden shadow-slate-200/50">
            ...
            {/* 🚩 V-CODE FIELD (RE-ADDED) */}
            <div className="relative z-10 pt-2 border-t border-slate-50">
               <InputField label="Verification Code (V-Code)" icon="🛡️" type={showPass ? "text" : "password"} value={device.v_code} onChange={(v:any) => handleChange('v_code', v)} light />
            </div>
          </div>
          ...
          <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl space-y-6">
            <div className="flex justify-between items-center px-1">
              ...
              <button type="button" onClick={handleGetLocation} disabled={isLocating} className="text-[10px] font-black bg-slate-900 text-white px-6 py-4 rounded-[20px] active:scale-0.95 shadow-lg flex items-center gap-2 transition-all">
          ...
          <div className="space-y-4">
            ...
            <textarea rows={4} value={device.device_notes || ""} onChange={e => handleChange('device_notes', e.target.value)} className="w-full p-7 bg-white border-2 border-slate-100 rounded-[40px] font-[1000] italic text-sm outline-none focus:border-blue-500 shadow-inner resize-none transition-all shadow-slate-100" placeholder="Important notes..." />
          </div>

          {/* 🚩 FINAL BUTTON (Non-Sticky) */}
          <div className="pt-10 pb-20 relative z-[150]">
            <button 
              type="button"
